...
namespace Task_App
{
    ...
    public sealed partial class TeamDetails : Page
    {
        ...
        public ObservableCollection<EmployeeDisplay> mem1 = new ObservableCollection<EmployeeDisplay>();
        ...
        private async void LoadTeams()
        {
            team.Clear();
            string tableCommand;
            ObservableCollection<Team> teams = await DataLayer.Load();
            if(teams.Count>0)
            {
                t = teams[0];
                foreach (var item in teams)
                {
                    ObservableCollection<members> head = await DataLayer.LoadingMembers(item.name, "designation");
                    if (head.Count > 0)
                        item.head = head[0].empname;
                    head = await DataLayer.LoadingMembers(item.name, "");
        ...
        private async void LoadMembersItems()
        {
            mem.Clear();
            mem1.Clear();
            mem = await DataLayer.LoadingMembers(t.name,"");
        ...
        private async void add_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (name == "" || type == "")
                return;
            await DataLayer.Insertteams(name, type, pd.emp.id, pd.emp.name);
        ...
        private async void add2_Click(object sender, RoutedEventArgs e)
        {
            ...
            string role1 = await DataLayer.GetDetails(name[1]);
            string[] role = role1.Split('-');
            
            bool result = await DataLayer.InsertToMembers(t.name, name[1], name[0], role[0] , role[1]);
            EmployeeDisplay ed = new EmployeeDisplay();
            ed.name = name[0];
            ed.role = role[0];
        ...
        private async Task<ObservableCollection<string>> LoadTeamMembers()
        {
            ...
            assign = await DataLayer.LoadEmployees();
