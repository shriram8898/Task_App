...
namespace Task_App
{
    ...
    public sealed partial class MembersDetails : Page
    {
        ...
        public ObservableCollection<EmployeeDisplay> emp2 = new ObservableCollection<EmployeeDisplay>();
        ...
        protected async override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            emp1 = await DataLayer.LoadEmployees();
            foreach(var item in emp1)
            {
                EmployeeDisplay ed = new EmployeeDisplay();
                ed.empid = item.id;ed.name = item.name;ed.designation = item.designation;ed.role = item.role;ed.username = item.username;
                string pic = "Assets/" + ed.empid + ".jpg";
        ...
        private async void add1_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (id1 == "" || name1 == "" || email1 == "" || role1 == "" || position1 == "")
                return;
            await DataLayer.InsertEmployee(id1, name1, email1, role1, position1);
            string pic = "Assets/" + id1 + ".jpg";
            emp2.Add(new EmployeeDisplay { empid = id1, name = name1, username = email1, role = role1, designation = position1,Img=new BitmapImage(new Uri(this.BaseUri,pic))});
            Addmembers.Hide();
        }
