...
namespace TaskApp.View
{
    ...
    public sealed partial class TaskDetailedView : Page,IRecordAudio
    {
        ...
        public async Task PlayAudioFile(string fileName)
        {
            foreach (var file in selection)
            {
                if (file.Name == fileName)
                {
                    ...
                    media_player.Source = MediaSource.CreateFromStorageFile(file);
        ...
        public async void RecordOrStop()
        {
            if (record.Content.Equals("Record"))
            {
                graph.Start();
                record.Content = "Stop";
            }
            else if (record.Content.Equals("Stop"))
            {
                graph.Stop();
                ...
                {
                    MessageDialog md = new MessageDialog("Failed to save.\nTry Again");
                    await md.ShowAsync();
                    return;
                }
                else
                {
                    MessageDialog md = new MessageDialog("Succefully saved");
                    await md.ShowAsync();
                    await tu.UpdateAudio(storageFile.Name,pd.Selected);
                    selection.Add(storageFile);
                    audio.Items.Add(storageFile.Name);
                    await CreateAudioGraph();
                    await SelectOutputFile();
                }
            }
        ...
        public async void OpenAudioFilePath()
        {
            ...
            foreach (var file in files)
            {
                ...
                string fileSize = string.Format("{0:n0}", basic.Size);
                if (fileSize == "0")
                {
                    file.DeleteAsync();
                }
                else
                {
                    ...
                    {
                        if(value.taskid==pd.Selected.id && value.file==file.Name)
                        {
                            audio.Items.Add(file.Name);
                            selection.Add(file);
                            break;
                        }
                    }                   
        ...
        {
            var Folder = ApplicationData.Current.LocalFolder;
            var folder = await Folder.CreateFolderAsync("Audio", CreationCollisionOption.OpenIfExists);
            storageFile = await folder.CreateFileAsync("audio.mp3", CreationCollisionOption.GenerateUniqueName);
            //var file = await ApplicationData.Current.LocalFolder.CreateFileAsync(name);
            if (storageFile == null)
            {
                return;
            }
            ...
            {
                MessageDialog md = new MessageDialog("Error in outputnode", "OOPS!");
                await md.ShowAsync();
                return;
            }
        ...
        private async void audio_ItemClick(object sender, ItemClickEventArgs e)
        {
            string name = e.ClickedItem.ToString();
            await PlayAudioFile(name);
